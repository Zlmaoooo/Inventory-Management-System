...
# ---------------------------------------------------------------------------
# Signal: atomically update Product.quantity after every Transaction save
# ---------------------------------------------------------------------------

@receiver(post_save, sender=Transaction)
def update_product_stock(sender, instance, created, **kwargs):
    """Atomically adjusts Product.quantity using F() to prevent race conditions.
    ...
    if instance.type == "IN":
        ...
        # Guard against going negative (validation happens in the form/view)
        Product.objects.filter(pk=instance.product_id).update(
            quantity=F("quantity") - instance.quantity
        )
